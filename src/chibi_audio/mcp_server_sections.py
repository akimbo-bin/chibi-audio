...
from .analysis_mcp import (
    AnalysisCapabilityList,
    AnalysisCostName,
    AnalysisDbfsThreshold,
    AnalysisFraction,
    AnalysisLabel,
    AnalysisLowBandHz,
    AnalysisPositiveMs,
    AnalysisSourceLabelList,
...
def build_mcp_server(
    ...
    *,
    ...
    if settings.allow_writes:
        ...
        @server.tool(
            title="Capture a named section and attribute master stress",
            description=(
                "Resolve one artist-authored Locator section, validate an explicit aligned tap layout, execute one "
                "managed ChibiTap capture with Set-signature fencing and exact topology restoration, then estimate "
                "latency-corrected master-chain stress against the requested source/group taps. The capture is a "
                "write-gated Live effect; the attribution is read-only evidence and does not prove causality."
            ),
            annotations=write_annotations,
            structured_output=True,
        )
        def capture_section_master_stress(
            name: ObjectName,
            experiment_id: ExperimentId,
            tap_specs: TapSpecList,
            ...
            try:
                if not tap_specs:
                    raise ValueError("capture_section_master_stress requires at least one tap spec")
                ...
                safe_premaster = _safe_id(premaster_label)
                safe_master = _safe_id(master_label)
                safe_sources = [_safe_id(value) for value in source_labels]
                requested_labels = [safe_premaster, safe_master, *safe_sources]
                if len(set(requested_labels)) != len(requested_labels):
                    raise ValueError("premaster, master and source labels must be distinct")
                ...
                for label in requested_labels:
                    ...
                    if len(matches) != 1:
                        raise ValueError(
                            f"capture_section_master_stress requires exactly one tap spec for label {label!r}"
                        )
                    selected[label] = matches[0]

                premaster_tap = selected[safe_premaster]
                if premaster_tap.target.strip().lower() != "master" or premaster_tap.signal_point != "pre_fx":
                    raise ValueError(
                        "premaster_label must select a master target captured at pre_fx"
                    )
                master_tap = selected[safe_master]
                if master_tap.target.strip().lower() != "master" or master_tap.signal_point != "post_fx":
                    raise ValueError(
                        "master_label must select a master target captured at post_fx"
                    )
                if any(selected[label].target.strip().lower() == "master" for label in safe_sources):
                    raise ValueError("source_labels must select non-master track/group taps")
                ...
                try:
                    analysis_result = analysis.attribute_capture_master_stress(
                        manifest_artifact,
                        premaster_label=safe_premaster,
                        master_label=safe_master,
