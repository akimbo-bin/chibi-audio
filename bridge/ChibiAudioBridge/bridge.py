...
def _temp_file(name):
    root = os.environ.get("CHIBI_AUDIO_STATE_DIR") or os.path.join(os.path.expanduser("~"), ".chibi-audio")
    try:
        os.makedirs(root)
    except OSError:
        pass
...
class AbletonLiveMCP(ControlSurface):
    ...
    def _start_server(self):
        ...
        thread.daemon = True
        thread.start()
        self.log_message("ChibiAudioBridge read-only pilot listening on %s:%s" % (HOST, PORT))
    ...
    def _dispatch(self, request):
        ...
        try:
            if method not in MODEL_EXPOSED_METHODS:
                raise ValueError("Method not exposed by Chibi Audio bridge policy: %s" % method)
            if method == "bridge_status":
                result = self._rpc_bridge_status(params)
            elif method == "agent_audio_tap":
                # Capture control is file/UDP I/O only; do not block on Live's main thread.
                result = self._rpc_agent_audio_tap(params)
            else:
    ...
    def _rpc_bridge_status(self, _params):
        ...
        return {
            ...
            "capabilities": {
                "read": list(MODEL_READ_METHODS),
                "bounded_write": list(MODEL_BOUNDED_WRITE_METHODS),
                "capture": list(MODEL_CAPTURE_METHODS),
                "gui_fallback": False,
    ...
    def _rpc_parameter_set(self, params):
        ...
        if len(parts) != 5 or parts[0] not in ("song", "live_set") or parts[1] != "tracks" or parts[3] != "mixer_device" or parts[4] != "volume":
            raise ValueError("parameter_set is restricted to exact track volume paths in this pilot")
        ...
        if not expected_name or getattr(track, "name", "") != expected_name:
            raise RuntimeError("Track identity mismatch for bounded volume write")
        param = self._resolve(ref)
        before = self._parameter_summary(param)
        expected_current = params.get("expected_current_value")
        if expected_current is None or abs(float(before.get("value")) - float(expected_current)) > 1e-6:
            raise RuntimeError("Track volume changed since inspection; refusing write")
    ...
    def _clip_summary(self, clip, slot_index):
        ...
        for attr in ("is_midi_clip", "is_audio_clip", "is_session_clip", "is_arrangement_clip", "position", "start_time", "end_time", "length", "loop_start", "loop_end", "muted", "has_envelopes"):
            try:
